...
def write_vectors_columnwise(out, qids, rels, docnos, columns):
    ...
    for i in range(nrows):
        ...
        row = " ".join(
            ["{}:{}".format(fid, val) for fid, val in enumerate(row_values, 1)]
        )
        print(
            "{} qid:{} {} # docno:{}".format(rels[i], qids[i], row, docnos[i]), file=out
        )
...
def cut(argv):
    ...
    parser.add_argument(
        "--renumbering", action="store_true", help="renumber the feature IDs"
    )
    parser.add_argument("vector_file", help="the input vector file")
    args = parser.parse_args(argv)
...
def join(argv):
    ...
    rows_list = [
        get_rows(_open(name), with_preamble=True) for name in args.vector_files
    ]
...
def shuffle(argv):
    ...
    parser.add_argument("-seed", help="use a custom seed instead of the system default")
    parser.add_argument("vector_file", help="input vector file")
    args = parser.parse_args(argv)
...
def split(argv):
    ...
    parser.add_argument("-k", type=int, help="number of folds (default: %(default)s)")
    parser.add_argument(
        "--prefix",
        type=str,
    ...
    parser.add_argument(
        "-c", "--complete", action="store_true", help="output training sets as well"
    )
    parser.add_argument("vector_file", help="input vector file")
    ...
    if args.complete:
        training_files = [
            "{}.fold-{}_training".format(prefix, k + 1) for k in range(args.k)
        ]
...
def normalize(argv):
    ...
    parser.add_argument("-m", help="normalizaion method name")
    parser.add_argument("vector_file", help="input vector file")
    args = parser.parse_args(argv)
    ...
    for vector, m in get_vectors(rows):
        ...
        for fid in sorted(vector):
            new_value = (
                float(vector[fid] - min_values[m["qid"]][fid]) / gaps[m["qid"]][fid]
            )
