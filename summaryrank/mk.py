...
class SynonymOverlap(summaryrank.Feature):
    ...
    def compute(self, model):
        ...
        for text, m in model.load_topics("topics_term"):
            synonym_list = [
                self.wordnet_synonyms(term, include_term=True) for term in text.split()
            ]
            queries[m["qid"]] = [
                [stemmer(syn) for syn in syns] for syns in synonym_list
            ]

        for text, m in model.load_sentences("sentences_stem"):
            stems = [s.decode("utf8") for s in text.split()]
            overlap = len(
                [1 for syns in queries[m["qid"]] if any([syn in stems for syn in syns])]
            )
            ratio = float(overlap) / len(queries[m["qid"]]) if overlap > 0 else float(0)
            result.append(ratio)
        return result
...
class LanguageModelScore(summaryrank.Feature):
    ...
    def __init__(self, args):
        ...
        self.mu = args.lm_mu
        self._freq_stats = (
            GalagoIndex(args.index, "postings.krovetz") if args.index else None
        )
    ...
    def init_parser(cls, parser, group):
        ...
        if not parser._get_option_tuples("--index"):
            group.add_argument(
                "--index", metavar="PATH", help="the background Galago index"
            )
    ...
    def compute(self, model):
        ...
        for text, m in sentences_stem:
            ...
            for query_stem in queries[m["qid"]]:
                ...
                score += math.log(
                    float(
                        sentence_tf[query_stem] + self.mu * float(cf) / collection_len
                    )
    ...
    def compute(self, model):
        ...
        for text, m in model.load_sentences("sentences_stem"):
            ...
            for query_stem in queries[m["qid"]]:
                ...
                comp3 = sentence_tf[query_stem] + self.k1 * (
                    1 - self.b + float(self.b * sentence_len) / self.avgdl
                )
...
def gen_term(argv):
    ...
    parser.add_argument(
        "--stemmer", help="use the specified stemmer: porter or krovetz (default)"
    )
    parser.add_argument(
        "-m",
        dest="model",
    ...
    with model.open("topics_term", "wb") as out_t, model.open(
        "topics_stem", "wb"
    ) as out_s:
    ...
    with model.open("sentences_term", "wb") as out_t, model.open(
        "sentences_stem", "wb"
    ) as out_s:
