...
class ESACosineSimilarity(summaryrank.Feature):
    ...
    def compute(self, model):
        ...
        norms = dict(
            (qid, self.norm_over_logarithms(vec)) for qid, vec in queries.items()
        )
        ...
        for rep, m in model.load_sentences("sentences_esa"):
            ...
            if query_norm > 0 and sentence_norm > 0:
                score = (
                    self.dot_product_over_logarithms(sentence, query)
                    / query_norm
                    / sentence_norm
                )
            result.append(score)
...
class Word2VecSimilarity(summaryrank.Feature):
    ...
    def init_parser(cls, parser, group):
        group.add_argument(
            "--word2vec-model", metavar="FILE", help="the word2vec binary model"
        )
    ...
    def check(self, model):
        ...
        with ElapsedTimeIndicator(
            "load " + self._word2vec_model + " [{elapsed}]"
        ) as indicator:
...
def gen_esa(argv):
    ...
    parser.add_argument("index_path", help="path to a Galago index")
    ...
    with model.open("topics_esa", "wb") as out:
        ...
        p = subprocess.Popen(
            [
                "galago",
                "batch-search",
                "--index={}".format(args.index_path),
                "--requested={}".format(args.k),
                query_filename,
            ],
    ...
    with model.open("sentences_esa", "wb") as out:
        ...
        with tempfile.NamedTemporaryFile(delete=False) as query_json:
            ...
            for docno, id_, qid, text in model.load_representation("sentences_term"):
                queries.append(
                    {"number": "{}:{}:{}".format(qid, docno, id_), "text": text}
                )
            json.dump({"queries": queries}, query_json, indent=2)
            query_filename = query_json.name
...
def gen_tagme(argv):
    ...
    parser.add_argument("api_key", help="TAGME API key")
    args = parser.parse_args(argv)
