...
def check_consistency(argv):
    ...
    parser = argparse.ArgumentParser(
        ...
        description="".join(
            [
                line.lstrip()
                for line in check_consistency.__doc__.strip().splitlines(True)[2:]
            ]
        ),
        add_help=False,
    )
    ...
    parser.add_argument(
        "-c", dest="charset", help="file encoding (default: %(default)s)"
    )
    parser.add_argument("argv", type=str, nargs="*", help=argparse.SUPPRESS)
    ...
    for sentence, metadata in trectext.sentences():
        if current_qid != metadata["qid"]:
            if pool:
                for group in pool.values():
                    ...
                    if all([s["rel"] == group_rel for s in group]):
                        continue
                    ...
                    if args.majority_fix:
                        ...
                        for s in group:
                            print(
                                "{docno}\t{id}\t{rel}".format(
                                    docno=s["docno"], id=s["id"], rel=majority_rel
                                )
                            )
                    else:
                        for s in group:
                            print(
                                "{sentence_id:24s}\t{rel}\t{sentence}".format(
                                    **s
                                ).encode("utf8")
                            )
        ...
        pool[digest].append(
            {
                "sentence_id": "{docno}.{id}".format(**metadata),
...
def fix_trectext(argv):
    ...
    parser = argparse.ArgumentParser(
        ...
        description="".join(
            [
                line.lstrip()
                for line in fix_trectext.__doc__.strip().splitlines(True)[2:]
            ]
        ),
        add_help=False,
    )
...
def fix_trectext2(argv):
    ...
    for document, metadata in trectext.documents():
        ...
        for i, text_ in enumerate(texts):
            if (
                difflib.SequenceMatcher(None, text, text_).quick_ratio()
                > args.threshold
            ):
                true_score = difflib.SequenceMatcher(None, text, text_).ratio()
        ...
        if valid:
            summaryrank.trectext.print_document(
                document, metadata, charset=args.charset
            )
    print("</ROOT>")
...
def print_document(document, metadata, charset="utf8"):
    ...
    for sentence, sentence_metadata in document:
        ...
        print("<SENTENCE>{}</SENTENCE>".format(sentence).encode(charset))
    print("</{}>".format(RELEVANCE_LABELS[current_rel]))
    print("</TEXT>\n" "</DOC>")
