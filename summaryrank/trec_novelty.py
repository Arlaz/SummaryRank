...
def import_trec_novelty(argv):
    ...
    parser.add_argument(
        "corpus_file", help="corpus file, a tarball as distributed by TREC"
    )
    ...
    sentences = get_sentences(
        tarfile.open(args.corpus_file, "r:gz"),
        qids=qids,
        qrels=qrels,
        charset="latin-1",
    )
...
def get_qrels_set(iterable):
    ...
    return set(
        [tuple(re.split(r"[ :]", line.strip(), maxsplit=2)) for line in iterable]
    )
